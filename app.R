...
server <- function(input, output, session) {
  ...
  var.info <- function(col) {
    ...
    if(col == "Pesticide.Exposure") return("Agricultural pesticide use data from the Washington State Department of Health Washington Tracking Network (WTN) (2023)")
    
    if(col == "Racial.Residential.Segregation") return("Racial residential segregation, calculated using Duncan's multi-group dissimilarity index with 2020 race data by block group from the National Historical Geographic Information Systems data portal")
